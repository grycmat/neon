...
- **Composer**: Text composer with media attachments, alt text, polls, CW toggle, and visibility settings. Neon also registers as an Android share target — sharing text/links or one or more photos/videos from another app opens the composer pre-filled with that text and/or the media already uploading.
...
- **Text Size**: Small/Default/Large/Extra Large text-size picker in Settings > Appearance, independent of the OS font-scale setting, scaling every typography role app-wide. See the "Theming & Material You" section in `CLAUDE.md` for the implementation.
- **Icon Size**: Small/Default/Large/Extra Large icon-size picker in Settings > Appearance, independent of Text Size, scaling the reply/boost/favourite/bookmark/share/more icons on post cards and in the thread view. See the "Theming & Material You" section in `CLAUDE.md` for the implementation.
